...
// =======================================================================
// user.go: VerifyPassword (82.4%) - malformed argon2 hash branches
// =======================================================================

func TestVerifyPassword_InvalidArgon2Format_Cov2(t *testing.T) {
	// Too few parts after splitting by $
	result := VerifyPassword("test", "$argon2id$v=19$m=65536,t=1,p=4$salt")
	if result {
		t.Error("expected false for malformed argon2 hash (5 parts)")
	}
}

func TestVerifyPassword_InvalidBase64Salt_Cov2(t *testing.T) {
	result := VerifyPassword("test", "$argon2id$v=19$m=65536,t=1,p=4$!!invalid!!$hashval")
	if result {
		t.Error("expected false for invalid base64 salt")
	}
}

func TestVerifyPassword_InvalidBase64Hash_Cov2(t *testing.T) {
	salt := base64.RawStdEncoding.EncodeToString([]byte("somesalt"))
	result := VerifyPassword("test", "$argon2id$v=19$m=65536,t=1,p=4$"+salt+"$!!invalid!!")
	if result {
		t.Error("expected false for invalid base64 hash")
	}
}

func TestVerifyPassword_BcryptWrongPassword_Cov2(t *testing.T) {
	hash, err := HashPasswordBcrypt("correctpassword")
	if err != nil {
		t.Fatalf("HashPasswordBcrypt: %v", err)
	}
	if VerifyPassword("wrongpassword", hash) {
		t.Error("expected false for wrong bcrypt password")
	}
}

func TestHashPasswordBcrypt_VerifyRoundTrip_Cov2(t *testing.T) {
...
func TestGenerateAppPassword_Uniqueness_Cov2(t *testing.T) {
	p1, err := GenerateAppPassword()
	if err != nil {
		t.Fatalf("GenerateAppPassword: %v", err)
	}
	p2, err := GenerateAppPassword()
	if err != nil {
		t.Fatalf("GenerateAppPassword: %v", err)
	}
	if p1 == p2 {
