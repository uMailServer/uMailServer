...
// =======================================================================
// HashPasswordBcrypt (75.0%) - verify the function works with various inputs.
// The error path (bcrypt.GenerateFromPassword failure) is very hard to trigger.
// =======================================================================

// TestHashPasswordBcrypt_LongPassword tests bcrypt with a long password (bcrypt max is 72 bytes).
func TestHashPasswordBcrypt_LongPassword_Cov5(t *testing.T) {
	longPassword := strings.Repeat("a", 72)
	hash, err := HashPasswordBcrypt(longPassword)
	if err != nil {
		t.Fatalf("HashPasswordBcrypt with long password: %v", err)
	}
	if !verifyBcrypt(longPassword, hash) {
		t.Error("Expected long password to verify")
	}
}

// TestHashPasswordBcrypt_TooLongPassword tests bcrypt with a password exceeding 72 bytes.
func TestHashPasswordBcrypt_TooLongPassword_Cov5(t *testing.T) {
	tooLongPassword := strings.Repeat("a", 100)
	hash, err := HashPasswordBcrypt(tooLongPassword)
	if err == nil {
		// bcrypt silently truncates to 72 bytes in some implementations,
		// but golang.org/x/crypto/bcrypt returns an error for >72 bytes
		// Verify the hash works if it was created
		if !verifyBcrypt(tooLongPassword, hash) {
			t.Error("Expected password to verify against its own hash")
		}
	}
	// If it errored, that's also acceptable behavior
}

// TestHashPasswordBcrypt_SpecialChars tests bcrypt with special characters.
func TestHashPasswordBcrypt_SpecialChars_Cov5(t *testing.T) {
	password := "p@$$w0rd!#$%^&*()"
	hash, err := HashPasswordBcrypt(password)
	if err != nil {
		t.Fatalf("HashPasswordBcrypt with special chars: %v", err)
	}
	if !verifyBcrypt(password, hash) {
		t.Error("Expected special char password to verify")
	}
	if verifyBcrypt("wrong", hash) {
		t.Error("Expected wrong password to not verify")
	}
}

// TestHashPasswordBcrypt_EmptyPassword tests bcrypt with empty password.
func TestHashPasswordBcrypt_EmptyPassword_Cov5(t *testing.T) {
	hash, err := HashPasswordBcrypt("")
	if err != nil {
		t.Fatalf("HashPasswordBcrypt with empty password: %v", err)
	}
	if !verifyBcrypt("", hash) {
		t.Error("Expected empty password to verify against its hash")
	}
}

// =======================================================================
// GenerateAppPassword (75.0%) - exercise the happy path more thoroughly.
// The error path requires rand.Read to fail, which is very hard to trigger.
// =======================================================================

// TestGenerateAppPassword_Uniqueness tests that multiple app passwords are unique.
func TestGenerateAppPassword_Uniqueness_Cov5(t *testing.T) {
	passwords := make(map[string]bool)
	for i := 0; i < 10; i++ {
		pwd, err := GenerateAppPassword()
		if err != nil {
			t.Fatalf("GenerateAppPassword %d: %v", i, err)
		}
		if passwords[pwd] {
